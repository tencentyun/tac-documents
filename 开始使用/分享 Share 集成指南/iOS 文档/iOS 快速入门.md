移动开发平台（MobileLine）使用起来非常容易，只需要简单的 4 步，您便可快速接入移动分享服务。接入后，您即可获得我们提供的各项能力，减少您在开发应用时的重复工作，提升开发效率。
...
TACSocialShare引入 TACSocialQQ 模块，请参考该模块的配置，并添加配置文件 `tac_services_configurations_qq.plist`，并将文件添加进XCode工程中。文件内容如下：
